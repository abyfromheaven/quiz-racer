...
// Menggunakan 'ex' yang sudah disediakan global oleh CDN Excalibur
...
// --- GAME STATE ---
let currentGameMode = GameMode.SOLO;
...
const players = [
  { 
    ...
    currentQ: 0, 
    car: null, 
    ui: { question: 'p1-question', options: '.p1-opt', score: 'p1-score', label: 'p1-ui' } 
  },
  { 
    score: 0, 
    health: MAX_HEALTH,
    currentQ: 0, 
    car: null, 
    ui: { question: 'p2-question', options: '.p2-opt', score: 'p2-score', label: 'p2-ui' } 
  }
];
...
// Finish Line
function createCheckeredFinish(x) {
  const size = 40;
...
function loadQuestion(playerIdx) {
  const p = players[playerIdx];
  const q = window.QUESTIONS[p.currentQ];
...
function handleAnswer(playerIdx, selectedIndex) {
  ...
  if (selectedIndex === q.answer) {
    ...
    const ui = document.getElementById(p.ui.label);
...
// Listeners
document.getElementById('mode-solo').addEventListener('click', (e) => {
  console.log("Tombol Solo diklik");
  startMode(GameMode.SOLO);
});
document.getElementById('mode-multi').addEventListener('click', (e) => {
  console.log("Tombol Multi diklik");
  startMode(GameMode.MULTI);
});
